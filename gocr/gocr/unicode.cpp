...
const char *decode(wchar_t c, FORMAT type) {
  ...
  switch (type) {
    ...
    default: /* use UTF8 as default, test with xterm -u8 */
      ...
      /* disable warnings on VS-2010 which seems to have 16bit wchar only */
      /* echo "" | gcc -E - -dM | grep -i wchar # __WCHAR_MAX__ 2147483647 */
      /*  if undefined, than UTF8 for more than 16bit will not work */
      /*  do not now if __SIZEOF_WCHAR_T__ is defined on nonGCC compilers */
#if __WCHAR_MAX__ > 0x10000  /* skip compile warnings on 16bit systems */
