...
int line_deviation( struct box *box1, int j1, int j2 ) {
  ...
  if (j1!=j2) // 2017-03 start j1+1 and j1!=j2 added
  for (i=j1+1;;i++) { // do it for each vector between j1 and j2
   ...
   // for (i=j1;i!=j2;i=(i+1)%box1->num_frame_vectors[0]) {~}
    ...
    // d is rel. position on j1-j2 line -1024=j1 0=center +1024=j2
    if (d<=-1024) { x=r1x*1024; y=r1y*1024; }  // starting point
      else {
        if (d>=1024) { x=r2x*1024; y=r2y*1024; }  // end point
          else {
            x=(r1x+r2x+1)*1024/2+(d*(r2x-r1x))/2; // 1024 units
            y=(r1y+r2y+1)*1024/2+(d*(r2y-r1y))/2;
            /* we have the crossing point x,y now */
          }
      }
    dist=sq((x-r3x*1024)/(box1->x1-box1->x0+1+4))  // 2017-03 +2 (small fonts)
        +sq((y-r3y*1024)/(box1->y1-box1->y0+1+4)); // 0..2*sq(1024)
    // d is rel. position on j1-j2 line -1024=j1 0=center +1024=j2
    //                                             j1  i   j2     x[i] y[i]
    DBG( IFV fprintf(stderr,"\nDBG deviation j1-j2 %2d %2d %2d d %5.2f xy %3d %3d  %4.1f %4.1f dist %5d",
        j1,i,j2, d/1024., r3x-box1->x0,r3y-box1->y0,x/1024.-r3x,y/1024.-r3y,dist);)
 ...
 * 2018-09:
 *  thin fonts may have inner and outer vector nearest to rx,ry, 5x8.y
 *  
 */
int nearest_frame_vector( struct box *box1, int j1, int j2, int rx, int ry) {
   ...
   // 2017-03 fix j1,j2 >= max (old: j1,j2 > max)
   if (!box1->num_frames) return(-1);
   if (j1<0 || j1>=box1->num_frame_vectors[box1->num_frames-1] ||
       j2<0 || j2>=box1->num_frame_vectors[box1->num_frames-1]) {
...
//           > loop(box1->p,xr,yl,xr-xl,cs,0,LE) ) // +dx/8 ) &agrave; Nov03
     ...
     fprintf(stderr," yu=%d yl=%d xl=%d xr=%d",yu-y0,yl-y0,xl-x0,xr-x0);
	   ...
	   mod = TILDE;
        ...
        // if (sdata->holes.num) ... not in this range?
	   mod = RING_ABOVE;
...
/* 7x7
  ...
  .@@@@@.<-      ..0@@$.  
  @@@@@@@        .@@@@@3
  @@@@@@@        $@@@@@@  <- min_gray=101 b=0 w=233 (ToDo17: check mingray)
  @@@@@@@  <- 2  @@$@@@$
  @@.....  <- 1  1$
  @@@@@@@        .@$@@@$
  .@@@@@@<-      ..$@@@2  see tmp13/sslmozFP_Fi.png 8x9-font a0.x>dx/3?
*/
      if (aa[0][0]>x0+dx/3 || aa[0][1]>y0+dy/4) Break;
      if (aa[1][0]>x0+dx/3 || aa[1][1]<y1-dy/2) Break;
      ...
      if (box1->frame_vector[i1][0]>=x0+(dx+2)/3) Break; // 2017-03 tmp15/ssl
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4 (smallest seen is 5x6)
      ...
      if (box1->num_frames != 1) ad=97*ad/100; // excludes tiny holes 1810.rnd80
      if (box1->num_frames > 2) Break;  // excludes tiny holes 1810.rnd80
...
// ..@@@@;..<-
   ...
   for(ad=d=100;dx>2 && dy>4 ;){     // min 3x4
     ...
     if (box1->num_frames != 1) Break;  // excludes tiny holes 1810.rnd80
...
static wchar_t ocr0_M(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   pix *bp=sdata->bp;
   int	d,x,y,i0,i1,i2,i3,i4,i5,i6,i7,t1,
        hchar=sdata->hchar,gchar=sdata->gchar,
        x0=box1->x0,x1=box1->x1,y0=box1->y0,y1=box1->y1,cs=sdata->cs;
   int  dx=x1-x0+1,dy=y1-y0+1,	/* size */
   ...
   for(ad=d=100;dx>3 && dy>3;){	// dy<=dx nicht perfekt! besser mittleres
      ...
      d=2*sq(128/4);  /* half distance to the center, added 2018-09 */
      if (aa[3][2]>d/2) Break;  /* [2] = distance, ~dj..., upper right */
      if (aa[0][2]>d/2) Break;  /* upper left end */
      if (aa[1][2]>d/2) Break;  /* lower left end */
      if (aa[2][2]>d/2) Break;  /* lowerright end */
      // search 3 legs and 2 space between, [][3]=vector_index
      i1=nearest_frame_vector(box1, aa[0][3],aa[2][3], x0, y1); // leg1
      i3=nearest_frame_vector(box1,       i1,aa[3][3], x1, y1); // leg3
      i4=nearest_frame_vector(box1,       i1,      i3, x0, y0); // gap1
      i5=nearest_frame_vector(box1,       i1,      i3, x1, y0); // gap2
      i2=nearest_frame_vector(box1,       i4,      i5, (x0+x1)/2, y1); // leg2
      MSG(fprintf(stderr,"i1-5 %d %d %d %d %d",i1,i2,i3,i4,i5);)
      // 2018-09 ToDo: check essentials of 3 legs
      if (box1->frame_vector[i1][0] > x0+dx/4) Break; // leg1 not leftmost
      if (box1->frame_vector[i4][1] > y0+dy/2) Break; // gap1 too low
      if (box1->frame_vector[i5][1] > y0+dy/2) Break; // gap2 too low
      if (box1->frame_vector[i5][0] < x0+dx/2) Break; // gap2 too left
      if (box1->frame_vector[i5][0]
        < box1->frame_vector[i2][0]) Break; // gap2 more left than leg2
      // check right side  molten 'nt' of tmp13/sslmozFP.png
      i6=nearest_frame_vector(box1,       i3, aa[3][3], x1+dx/2, y0+dy/8);
      i7=nearest_frame_vector(box1,       i3,       i6, x0, y0+dy/2);
      if (box1->frame_vector[i7][0]<box1->frame_vector[i6][0]-dx/8 && hchar)
        Break; //  ad=97*ad/100;
      ...
      //if (sdata->holes.num != 0) Break;  // includes tiny holes 1810.rnd80
      if (box1->num_frames != 1) Break;  // excludes tiny holes 1810.rnd80
...
static wchar_t ocr0_N(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>3 && dy>3;){  // 4x6font
      ...
      /* half distance to the center */
      d=2*sq(128/4);   /* ToDo17 bug? add doc, [][2]=dist? square() */
...
*/
     ...
     MSG(fprintf(stderr,"i1-7 %d %d %d %d %d %d %d",i1,i2,i3,i4,i5,i6,i7);)
      ...
      if (box1->num_frames > 1) // excludes tiny holes 1810.rnd80
...
static wchar_t ocr0_H(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
     ...
     // if (sdata->holes.num > 1) Break; /* tolerant against a tiny hole */
     if (box1->num_frames != 1) ad=97*ad/100; // excludes tiny holes 1810.rnd80
     if (box1->num_frames > 3) break; // excludes tiny holes 1810.rnd80
      if ( num_cross(0,dx-1,dy/4  ,dy/4  ,bp,cs) != 2
   ...
   for(ad=d=100;dx>2 && dy>3;){     // Mar2000 perfect??? no ocr-a X Jul09
     ...
     printf("a=%d %d b=%d %d c=%d %d d=%d %d e=%d %d f=%d %d  dxdy %d %d",\
      ...
      if( dx>8 ){ // example szaka0103
        ...
        // ImageMagick.convert.textfont=FreeMono-Regular,80 bowed
        if( get_line2(xe,ye,xf,yf,bp,cs,100)<95 ) ad=99*ad/100; // right down
        if( get_line2(xe,ye,xf,yf,bp,cs,100)<80 ) Break; // right down
...
//      if (box1->num_frames != 3) Break; // excludes tiny holes 1810.rnd80
   ...
   for(ad=d=100;dx>3 && dy>4;){     // min 3x4
      ...
      // 2017-08 divided To to Tb ?
      i=loop(bp,0   ,dy/8     ,dx,cs,0,RI); if (i>dx/2) Break;
      i=loop(bp,i   ,dy/8     ,dx,cs,1,RI); j=i; // upper width
      i=loop(bp,0   ,2*dy/3   ,dx,cs,0,RI); if (i>dx/4) Break;
      i=loop(bp,i   ,2*dy/3   ,dx,cs,1,RI); // lower width
      DBG( IFV fprintf(stderr,"\nDBG b ij %3d %3d",i,j);)
      if (2*j<i) ad=98*ad/100; 
      if (!hchar) ad=99*ad/100;
      if ( gchar) ad=99*ad/100;
      Setac(box1,'b',ad);
      if (ad>=100) return 'b';
...
static wchar_t ocr0_dD(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      ...
      if( num_cross(x0 ,x1 ,y0+dy/3  ,y0+dy/3  ,box1->p,cs) != 2 
       && num_cross(x0 ,x1 ,y0+dy/3+1,y0+dy/3+1,box1->p,cs) != 2 ) Break;
...
static wchar_t ocr0_uU(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   pix *bp=sdata->bp;
   int	i1,i2,i3,i4,i,j,d,x,y,hchar=sdata->hchar,gchar=sdata->gchar,
        x0=box1->x0,x1=box1->x1,y0=box1->y0,y1=box1->y1,cs=sdata->cs;
   int  dx=x1-x0+1,dy=y1-y0+1,	/* size */
        ad;	/* tmp-vars */
   int (*aa)[4]=sdata->aa;    /* corner-points, (x,y,dist^2,vector_idx) */
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      DBG( wchar_t c_ask='u'; )
      //if (sdata->holes.num > 1) Break; /* tolerant against a tiny hole */
      if (box1->num_frames > 3) Break; // u+"u excludes tiny holes 1810.rnd80
      if (aa[0][0]>x0+dx/3 || aa[0][1]>y0+dy/4) Break; // left  top
      if (aa[1][0]>x0+dx/4 || aa[1][1]<y1-dy/4) Break; // left  down
      if (aa[2][0]<x1-dx/3 || aa[2][1]<y1-dy/4) Break; // right down
      if (aa[3][0]<x1-dx/4 || aa[3][1]>y0+dy/4) Break; // right top
...
/* V.8x10
   ...
   ..@..
 */
      // upper gap from the top      2018-09
      i1=nearest_frame_vector(box1,aa[3][3],aa[0][3], x0+dx/2, y1);
      // lower gap from bottom (handwritten u?)
      i2=nearest_frame_vector(box1,aa[1][3],aa[2][3], x1-dx/4, y1-dy/4);
      if (box1->frame_vector[i1][1]<y1-dy/4) ad=99*ad/100; // 2018-09
      if (box1->frame_vector[i2][1]<y1-dy/4) ad=99*ad/100; // 2018-09
      MSG(fprintf(stderr,"i12 %d %d ad=%d",i1,i2,ad);)
      if (box1->frame_vector[i1][1]<y1-dy/3) Break; // 2018-09
      if (box1->frame_vector[i2][1]<y1-dy/3) Break; // 2018-09
      ...
      for(i=dy/8+2,y=dy/8;y<dy-(dy+2)/4 && i;y++){	// 12%+1 Fehler
        j=num_cross(0,dx/2-((y>dy/2)?dx/8:0),y,y,bp,cs); // left vert line
        if( y<dy/2 && num_cross(dx/2,dx-1,y,y,bp,cs)>1 ) i--; // ~{\it v}
        if( y<dy/2 && (j<1 && j>2) ) { i--; ad=90*ad/100; }
        if( y>dy/2 && j!=1 ) { i--; ad=98*ad/100; // handwritten u?
           MSG(fprintf(stderr,"ad=%d",ad);) }
      } if( !i ) Break;
      ...
      if ( loop(bp,dx/2,dy-1,dy,cs,0,UP)>dy/16 ) ad=98*ad/100; // w
...
static wchar_t ocr0_vV(ocr0_shared_t *sdata){
   ...
   // --- test v -------------------------------------------------
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4, ToDo: replace by vector-code
      DBG( wchar_t c_ask='v'; )
 ...
 */
      ...
      if (!hchar // 2010-10
       && loop(bp,0   ,dy/6     ,dx,cs,0,RI)
        >=loop(bp,0   ,dy-1-dy/3,dx,cs,0,RI) // old Break;
       && loop(bp,0   ,dy/6     ,dx,cs,0,RI)  // 2017-03-23 for 7x7 DOS-v
        >=loop(bp,0   ,dy-1-dy/4,dx,cs,0,RI) && dy>6 ) Break;
      if ( hchar
      ...
      if (!hchar  // 2010-10
       && loop(bp,dx-1,dy/6     ,dx,cs,0,LE)
        >=loop(bp,dx-1,dy-1-dy/3,dx,cs,0,LE) 
       && loop(bp,dx-1,dy/6     ,dx,cs,0,LE) // 2017-03 add 7x7-DOS-v
        >=loop(bp,dx-1,dy-1-dy/4,dx,cs,0,LE) && dy>6 ) Break;
      ...
      i=loop(bp,  i,   0,dx,cs,1,RI); i1=i; // thickness upper left
      i=loop(bp,  0,   1,dx,cs,0,RI);
      i=loop(bp,  i,   1,dx,cs,1,RI); if(i>i1) i1=i; // thiggest
      i=loop(bp,  0,dy/4,dx,cs,0,RI);
      i=loop(bp,  i,dy/4,dx,cs,1,RI); i2=i; // thickness on 1/4 from up
      i=loop(bp,  0,dy/4+dy/32,dx,cs,0,RI);
      i=loop(bp,  i,dy/4+dy/32,dx,cs,1,RI); if (i>i2) i2=i; // fat fonts
      i=loop(bp,  0,dy-1,dx,cs,0,RI);
      i=loop(bp,  i,dy-1,dx,cs,1,RI); i3=i; // thickness, fix dy-1 2013-06
      i=loop(bp,  0,dy-2,dx,cs,0,RI);
      i=loop(bp,  i,dy-2,dx,cs,1,RI); if(i>i3) i3=i; // thiggest bottom
      if( y0 < box1->m2)
      if( i1-i2 > dx/32+2 
       && i3-i2 > dx/32+2 ) {
        /* tmp10/invalid_ogv.jpg 120x145 font may have jpg-artefacts */ 
...
static wchar_t ocr0_m(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   pix *bp=sdata->bp;
   const int  hchar=sdata->hchar,gchar=sdata->gchar;
   int	i,d,x,y,i1,i2,i3,i4,i5,i6,i7,
        handwritten=0,
        x0=box1->x0,x1=box1->x1,y0=box1->y0,y1=box1->y1,cs=sdata->cs;
   int  dx=x1-x0+1,dy=y1-y0+1,	/* size */
        (*aa)[4]=sdata->aa,    /* corner-points, (x,y,dist^2,vector_idx) */
   ...
   for(ad=d=100;dx>4 && dy>3;){
      ...
      if (box1->num_frames != 1) Break; // excludes tiny holes 1810.rnd80
      // added 2018 tmp13/sslmozFP.png molten "ity"
      d=2*sq(128/4);  /* half distance to the center */
      // if (aa[3][2]>d/2) Break;  /* [2] = distance, ~dj... */
      if (aa[0][2]>d/2) Break;  /* upper left end */
      if (aa[1][2]>d/2) Break;  /* lower left end */
      if (aa[2][2]>d/2) Break;  /* lowerright end */
      ...
      MSG(fprintf(stderr,"i1-7 %d %d %d %d %d %d %d ad=%d",\
           i1,i2,i3,i4,i5,i6,i7,ad);)
      // check up side  molten 'ity' of tmp13/sslmozFP.png
      i6=nearest_frame_vector(box1, aa[3][3], aa[0][3], x0+dx/2, y1);
      if (box1->frame_vector[i6][1]>y0+dy/4 && hchar && gchar) Break;
       //  ad=97*ad/100;
...
static wchar_t ocr0_tT(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   pix *bp=sdata->bp;
   int	i,i1,i2,i3,i4,i5,i6, j,d,d2,x,y,yb,
        hchar=sdata->hchar,gchar=sdata->gchar,
        x0=box1->x0,x1=box1->x1,y0=box1->y0,y1=box1->y1,cs=sdata->cs;
   int (*aa)[4]=sdata->aa;    /* corner-points, (x,y,dist^2,vector_idx) */
   int  dx=x1-x0+1,dy=y1-y0+1,	/* size */
   ...
   for(ad=d=100;dx>2 && dy>3;){     // dx>1 dy>2*dx 
      ...
      /* half distance to the center */
      d=2*sq(128/4); /* square */
      /* now we check for the 3 edges of the T, but failes glued tmp13/ssl */
      if (aa[0][2]>d) Break; /* square distance to upper left  corner */
      if (aa[3][2]>d) Break; /* square distance to upper right corner */
      if (aa[3][0]-aa[0][0]<dx/2) Break; /* upper line to short? */
      // FreeMono-Regular with serif has -10+3+10=23 lower serif dx=36 64%
      if (aa[2][0]-aa[1][0]>2*dx/3) Break; /* lower line to long? ~t 7x9 */
      if (aa[1][1]-aa[0][1]<dy/2) Break; /* left  y-distance to short? */
      if (aa[2][1]-aa[3][1]<dy/2) Break; /* right y-distance to short? */
      if (aa[3][0]-aa[0][0]<4-1) Break; /* to small to hold a T */
      if (aa[3][1]-y0>dy/8) ad=99*ad/100;
      if (aa[0][1]-y0>dy/8) ad=99*ad/100;
      if (aa[2][0]>=aa[3][0])   ad=98*ad/100; // ~t 7x9
      if (aa[2][0]>=aa[3][0]-1) ad=99*ad/100; // ~t 7x9
      // detect bottom end of vertical line  2018-09
      i1=nearest_frame_vector(box1, aa[0][3],aa[2][3], x0, y1+dy/8);
      i2=nearest_frame_vector(box1, aa[1][3],aa[3][3], x1, y1+dy/8);
      // detect top    end of vertical line
      i3=nearest_frame_vector(box1, aa[0][3],aa[1][3], x1, y0);
      i4=nearest_frame_vector(box1, aa[2][3],aa[3][3], x0, y0);
      // serifen at bottom?  if i1==i6 + i2==i7 no serifen, ad*99%?
      i5=nearest_frame_vector(box1, i3, i1, x1, y1); // left
      i6=nearest_frame_vector(box1, i2, i4, x0, y1); // right
      if (box1->frame_vector[i3][0]-x0 > 3*dx/4) Break; // ~7
      if (box1->frame_vector[i4][0]-x0 >= dx-dx/8) Break; // ~7
      if (box1->frame_vector[i3][1]
       != box1->frame_vector[i4][1]) { ad=99*ad/100; MSG({}) } // 
      if (   box1->frame_vector[i3][0]-x0
       >  2*(x1-box1->frame_vector[i4][0])) Break; // ~7
      MSG(fprintf(stderr,"i1-6 %d %d %d %d %d %d ad %d",i1,i2,i3,i4,i5,i6,ad);)
      
      // old pixel code ... (ToDo replace)
      ...
      // ToDo17: pixel version fails on cutted fat "To"
      if( get_bw(dx-1-dx/4,dx-1,     dy/2,dy-1-dy/4,bp,cs,1) == 1 ) {ad=98*ad/100;MSG({}) }
      if( get_bw(dx-1-dx/5,dx-1,     dy/2,dy-1-dy/4,bp,cs,1) == 1 ) Break;
      // top width
      ...
      if (box1->m3 && 2*y1>box1->m3+box1->m4
        ...
        && loop(bp,0,dy-1,dy  ,cs,0,UP)<=dy/2) { ad=96*ad/100; MSG({}) }// ~J
      if (gchar) ad=98*ad/100;
      if( loop(bp,0,dy-1,dx,cs,0,RI)<=dx/8) { ad=99*ad/100; MSG({}) }// ~J
      i = loop(bp,0,dy/2,dx,cs,0,RI); // middle of vert. line
      j = loop(bp,i,dy/2,dx,cs,1,RI); // thickness
      if( 2*i>/*=*/dx ) { ad=99*ad/100; MSG({}) }
      if( 2*(dx-j-i)<i) {
        ad=95*ad/100; MSG(fprintf(stderr,"ij1 %2d %2d",i,j);) } // ~J
   ...
   for(ad=d=100;dx>1 && dy>3/*&& dy>=box1->m3-box1->m2-1 2017_7x9*/;){  // sometimes no hchar!
      DBG( wchar_t c_ask='t'; )
      //if (sdata->holes.num > 1) Break; /* tolerant against a tiny hole */
      if (box1->num_frames != 1) Break; // excludes tiny holes 1810.rnd80
      if (dy<=box1->m3-box1->m2+1) ad=96*ad/100; // bad line detection?
      if (box1->num_frames>1){ 
        if (4*box1->frame_vol[1]<box1->frame_vol[0]) Break; // ~! dosemu
        ad=ad*99/100; // ~! 2017_dosemu_4x10
      }
      /* old pixel code ... ToDo17 replace */
      ...
      if (x>=dx && 9*dx>=8*dy) { ad=99*ad/100; MSG({}) } // +
      
      if (box1->y1 < box1->m3 - (box1->m3-box1->m2+1)/32
        // ToDo && no bottom serif && h-line near (m2+m3)/2, not m2 = "+"
        && x0+loop(bp,dx/4,0,dy,cs,0,DO) >= (3*box1->m2+box1->m3)/4
        &&    loop(bp,0,dy/4,dx,cs,0,RI) >= dx/2-dx/8
        &&    10*dx>=8*dy
       )  { ad=98*ad/100; MSG({}) } // +
      ...
      MSG(fprintf(stderr,"b.y %2d",yb);)
      i=loop(bp,dx-1,yb,dx,cs,0,LE); // yb = upper cross line
      for(y=dy/8;y<yb;y++)
      if( loop(bp,dx-1,y,dx,cs,0,LE)>i ) break;
      if( y==yb && yb>dy/8) Break; // 2017-07 fix 6x8t on 7x9-font
     ...
     if( num_cross(0,dx-1,3*dy/4,  3*dy/4,  bp,cs) >= 2
      ...
      if (yb>1) {i= loop(bp,0,   1,dx,cs,0,RI); if (i<x) x=i;} // fix 6x8t
      i= loop(bp,0,dy/4,dx,cs,0,RI);
      if (i-x>1) Break; // l
...
static wchar_t ocr0_gG(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>4;){     // min 3x5
      ...
      if (sdata->holes.num > 2) ad=99*ad/100;
      if (sdata->holes.num < 1) Break;
      // ToDo: if 2 holes, loewer hole should be below baseline ~8
      ...
      for (i=0;i<sdata->holes.num;i++){ // check for upper hole
        if (sdata->holes.hole[i].y1 < 5*dy/8+1) break;
      } if (i==sdata->holes.num) Break; // no upper hole found
      if (sdata->holes.num>1){ // 2018-09 check for low hole
       for (i=0;i<sdata->holes.num;i++){ 
        if (sdata->holes.hole[i].y0 >= box1->m3-dy/4-y0) break;
       } if (i==sdata->holes.num) Break; } // no lower hole found 2018-09 nums1
      ...
      if ( hchar) ad=96*ad/100;
      if (!gchar) ad=96*ad/100;  // tmp12/nums1.jpg '8' ?
      ad=98*ad/100;
      Setac(box1,'g',ad);
      break;
   }
   ...
   for(ad=d=100;dx>3 && dy>4;){     // min 3x4
      ...
      turmite(box1->p,&x,&y,x0,x1,y0+dy/3,y1,cs,LE,UP); // w=LE b=UP
      // MSG(fprintf(stderr,"xy= %d %d",x-x0,y-y0);)
      if( y>y0+dy/3+1 ) Break; /* leave box below for S or on top for CG */
...
static wchar_t ocr0_yY(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   pix *bp=sdata->bp;
   int	i1,i2, i,j,d,x,y,hchar=sdata->hchar,gchar=sdata->gchar,
        x0=box1->x0,x1=box1->x1,y0=box1->y0,y1=box1->y1,cs=sdata->cs;
   int  dx=x1-x0+1,dy=y1-y0+1,	/* size */
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
     ...
     if (sdata->holes.num > 1) Break; /* tolerant against a tiny hole */
     if (sdata->holes.num > 0) ad=97*ad/100;
     /* half distance to the center, added 2018-09 */
     d=2*sq(128/4);
     /* now we check for the upper right end of the h */
     if (aa[3][2]>d/2) Break;  /* [2] = distance, ~N... */
     if (aa[0][2]>3*d/4) Break;  /* upper left end, handwritten tolerance */
     if (aa[1][2]<d/4          /* lower left end */
      && aa[2][2]<d/4) ad=99*ad/100; /* lowerright end */
     i1=i2=-1;
     if (aa[1][2]<(d+2)/4 
      && aa[2][2]<(d+2)/4) { // 2018-09 tmp12/rasterchars_small.png ~N
       i1=nearest_frame_vector(box1,aa[1][3],aa[2][3],x0,y0); // left N-gap?
       if (box1->frame_vector[i1][1]-y0<=(dy+2)/4
        && box1->frame_vector[i1][0]-x0<=(dx+2)/4) Break;
     }
     if (aa[2][2]<(d+2)/4
      && aa[0][2]<(d+2)/4) { // 2018-09 tmp12/rasterchars_small.png ~N
       i2=nearest_frame_vector(box1,aa[3][3],aa[0][3],x1,y1); // right N-gap?
       if (y1-box1->frame_vector[i2][1]<=(dy+2)/4
        && x1-box1->frame_vector[i2][0]<=(dx+2)/4) Break;
     }
     MSG(fprintf(stderr,"i1-2 %d %d ad=%d",\
           i1,i2,ad);)
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
     ...
     MSG(fprintf(stderr,"i1-2 %d %d ad=%d",\
      ...
      if( num_cross(0,dx-1,dy-1,dy-1,bp,cs) != 1 
       && num_cross(0,dx-1,dy-2-dy/32,dy-2-dy/32,bp,cs) != 1 ) Break;
      if( num_cross(dx-1,dx-1,0,dy-1,bp,cs) != 1 
...
static wchar_t ocr0_zZ(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>3 && dy>3;){     // dy>dx
      ...
      if (aa[0][2]>d) Break; /* square distance to upper left  corner */
      if (aa[1][2]>d) Break; /* square distance to lower left  corner */
      if (aa[2][2]>d) Break; /* square distance to lower right corner */
      if (aa[3][2]>d) Break; /* square distance to upper right corner */
      if (aa[3][0]-aa[0][0]<dx/2) Break; /* upper line to short? */
      if (aa[2][0]-aa[1][0]<dx/2) Break; /* lower line to short? */
      if (aa[1][1]-aa[0][1]<dy/2) Break; /* left  y-distance to short? */
      if (aa[2][1]-aa[3][1]<dy/2) Break; /* right y-distance to short? */
      ...
      /* output rel. koords of aa[0..3] */
      MSG( \
        fprintf(stderr,"aa[0-3].xy %d %d  %d %d  %d %d  %d %d ad %d", \
      ...
      MSG(fprintf(stderr,"i23xy= %d %d  %d %d  ad %d",x-x0,y-y0,\
        box1->frame_vector[i3][0]-x0,box1->frame_vector[i3][1]-y0,ad);)
      ...
      /* search highest left < from right side 2017-07 ~I 5x9 */
      i2=nearest_frame_vector(box1,aa[2][3],aa[3][3], x0, y0);
      x=box1->frame_vector[i2][0];
      y=box1->frame_vector[i2][1];
      if (y-y0 <= dy/8 && x-x0 <= 5*dx/8) Break; // ~I 2017-07_clean5x9
      if (y-y0 <= dy/4 && x-x0 <= 5*dx/8) ad=98*ad/100; // ~I
      ...
      if (y-y0 < 3*dy/8) Break;
      if (x-x0 > 5*dx/8) Break;
      if (x-x0 >= dx/2) ad=99*ad/100; // ~I 2017-07_clean5x9
      ...
      MSG(fprintf(stderr,"left diag line? i2xy %d %d d= %d krit= 2..3*%d",\
          x-x0,y-y0,d,sq(1024/4));) 
...
// ToDo: MSG(fprintf(stderr,"x,y,i4,i3= %d %d %d %d",x,y,i4,i3);)
   ...
   // new vector based fat a variant without holes 2010-10-11
   // --- test a ---------------------------------------------------
   for(ad=d=100;dx>2 && dy>4;){     // min 3x5
      DBG( wchar_t c_ask='a'; )
      if (sdata->holes.num > 1)  break;
...
*/
   ...
   for(ad=d=100;dx>3 && dy>3;){     // min 4x4
      ...
      i = loop(bp,dx/2-dx/8, 0 ,dy,cs,0,DO); if (i>dy/4) Break; // top center
      i+= loop(bp,dx/2-dx/8, i ,dy,cs,1,DO); if (i>dy/2) Break; // top hole
      i = loop(bp,dx/2-dx/8, i ,dy,cs,0,DO); if (i<dy/4) Break; // hole hight
 ...
 */       
      ...
      if(dx<8 && dy<12){ // screen font
        ...
        if ( loop(bp,dx/2,1,dy,cs,1,DO)>=dy-2
          ...
          && i>1 && i<dy/2 ) Break; // ~1
        // tmp17/0506_clean7x9b.png ~1 2017-08
        if ( 3*(loop(bp,0,dy-dy/8-2,dx,cs,0,RI)
               -loop(bp,0,dy     -1,dx,cs,0,RI)) >= dx) ad=98*ad/100;
      }
...
//      if( vertical_detected && dx>5 )
      ...
      if (loop(bp,0,dy/4,dx,cs,0,RI)>dx/2
       && loop(bp,0,7*dy/8,dx,cs,0,RI)<dx/2){ // ~z example?
        i=loop(bp,0,dy/16  ,dx,cs,0,RI);
        i=loop(bp,i,dy/16  ,dx,cs,1,RI); j=i; // top width
        i=loop(bp,0,dy/16+1,dx,cs,0,RI);
        i=loop(bp,i,dy/16+1,dx,cs,1,RI); if (i>j) j=i; // top max width
        i=loop(bp,0,dy/16+2,dx,cs,0,RI);
        i=loop(bp,i,dy/16+2,dx,cs,1,RI); if (i>j) j=i; // top max width
        if (j*4>=dx*3) ad=98*ad/100; // ~z
        if (j*8>=dx*7) ad=96*ad/100; // ~z
        MSG( fprintf(stderr,"ad= %d", ad); )
        // false z on 7x9font 3x9 l (left vertical + upper stub)
      }
      i=loop(bp,0,0,dy,cs,0,DO);
      if (3*i>dy && 3*i<2*dy
...
static wchar_t ocr0_oO(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      ...
      if( loop(bp,0   ,0   ,x1-x0,cs,0,RI)
         -loop(bp,0   ,2   ,x1-x0,cs,0,RI)<=dx/16 ) ad=99*ad/100;
      if( loop(bp,0   ,0   ,x1-x0,cs,0,RI)
         -loop(bp,0   ,2   ,x1-x0,cs,0,RI)<=dx/8  ) ad=98*ad/100;
      if( loop(bp,0   ,0   ,x1-x0,cs,0,RI)<
          loop(bp,0   ,2   ,x1-x0,cs,0,RI)  ) Break;
      MSG(fprintf(stderr,"ad %d",ad);)
      ...
      if(     loop(bp,0,     dy/16,dx,cs,0,RI)
         ...
         -  2*loop(bp,0,     dy/2 ,dx,cs,0,RI) < 0 ) Break;
      if(     loop(bp,0,     dy/16,dx,cs,0,RI)
         +    loop(bp,0,dy-1-dy/16,dx,cs,0,RI)
         -  2*loop(bp,0,     dy/2 ,dx,cs,0,RI) <= dx/8 )
         { if (hchar) {Break;} else ad=98*ad/100; } // not konvex
...
//     && ( get_bw(x0,x0+dx/32,y0,y0+dy/32,box1->p,cs,1) == 1
      ...
      // search lowest inner white point, set i=y itallic a
      for(y=dy,j=x=0+dx/8;x<dx-dx/8;x++) {
        i =loop(bp,x,dy-1  ,y1-y0,cs,0,UP);
        i+=loop(bp,x,dy-1-i,y1-y0,cs,1,UP);
        if (i<=y) { y=i; j=x; }
      } i1=y;
      // search highest inner white point, set i=y itallic a
      for(y=dy,j=x=dx/8;x<dx-dx/8;x++) {
        i =loop(bp,x,0  ,y1-y0,cs,0,DO);
        i+=loop(bp,x,0+i,y1-y0,cs,1,DO);
        if (i<=y) { y=i; j=x; }
      } i2=y;
      MSG(fprintf(stderr,"y12 %d %d ad %d",i1,i2,ad);)
      // italic a
      for(y=dy-1-i1;y<dy-1;y++)
        if( num_cross(j,dx-1,y,y,bp,cs) > 1 ) ad=99*ad/100; // ~a \it a
      MSG(fprintf(stderr,"i12 %d %d ad %d",i1,i2,ad);)
      for(y=0;y<i2;y++)
        if( num_cross(0,dx-1,y,y,bp,cs) > 2 ) ad=98*ad/100; // ~a \it a
      MSG(fprintf(stderr,"i12 %d %d ad %d",i1,i2,ad);)
      for(y=i1+dy/8;y<i2-dy/8;y++)
        if( num_cross(0,dx-1,y,y,bp,cs) > 2 ) ad=98*ad/100; // 0 Q ?
      MSG(fprintf(stderr,"i12 %d %d ad %d",i1,i2,ad);)
      if (loop(bp,dx-1,dy-1,x1-x0,cs,0,LE)<dx/8) ad=98*ad/100; // \it a
      if (loop(bp,dx-1,   0,x1-x0,cs,0,LE)<dx/8) ad=98*ad/100; // \it a
      MSG(fprintf(stderr,"ad %d m14 %d %d",ad,box1->m1-y0,box1->m4-y0);)
      ...
      if (abs(loop(bp,dx/2,   0,dy,cs,0,DO)
        ...
        || num_cross(0,dx-1,dy-1,dy-1,bp,cs) > 1
         ) { ad=98*ad/100; MSG(fprintf(stderr,"ad %d",ad);) }// ~bq

      // corrections for wrong recognized m1,m2 (all chars of same high)
      if (hchar && 2*y0<box1->m1+box1->m2) i=1; else i=0;
      if (gchar) ad=99*ad/100;
      bc='o';   // ToDo: need line information, if m1-m4 is not sure
      if (i){ bc='O'; }
      if ( bc=='O' && ad>99) ad=99; /* we can never 100% sure, 0O */
      if (bc=='O' && hchar && dy<10 && ad>=99 && dx<dy-dy/16 &&
        6*( loop(bp,   0,0,dx,cs,0,RI)
           +loop(bp,dx-1,0,dx,cs,0,LE))>=4*dx) { 
             Setac(box1,'0',99);
             Setac(box1,'O',98);Break; } // small top 0
      if (bc=='O' && hchar && dy<10 && ad>=99 && dx>=dy-dy/16) {
             Setac(box1,'0',98); // but output as 'o'
             Setac(box1,'O',99);Break; } // round O sslmozFP
      /* 2017: upper width: 3/5 for 5x7o, 2/6 for 6x9zero  but teletext!*/
      if (bc=='o' && (!hchar) && y0<=box1->m2) { // 2018-09 bad m1-m4?
        DBG( c_ask='O'; )
        Setac(box1,'O',99*ad/100);
        DBG( c_ask='0'; )
        Setac(box1,'0',99*ad/100);
      }
...
//  may be its better to trust the frame melting algorithm (ad=96%=ok)
   ...
   for(ad=d=100;dy>4 && dx>0;){     // min 3x4
      ...
      if (sdata->holes.num > 1) Break; /* tolerant against a tiny hole */
      // ToDo frames > 2 Break
      if (dx==1 && !gchar) Break;  /* 2017-03 sure not a 'j', old=96% */
      ya=y0;
      if( box1->m2 && 2*y0>=box1->m2+box1->m1 ) ya=box1->m1;
      ...
      if( 2*y0 <= box1->m1+box1->m2
       && loop(bp,0,   0,dx,cs,0,RI)+1
        < loop(bp,0,dx/2,dx,cs,0,RI)   )
         { ad=97*ad/100;  MSG(fprintf(stderr,"ad=%d",ad);) }
//      if (loop(bp,0,dy-1,dx,cs,0,RI)
//         -loop(bp,0,dy-3,dx,cs,0,RI)>1+dx/16)
//         { ad=96*ad/100;  MSG(fprintf(stderr,"ad=%d",ad);) } // ~c
   ...
   for(ad=d=100;dy>4 && dy>dx && 5*dy>4*(box1->m3-box1->m2);){     // min 3x4
      ...
      for(i=0,y=(dy+7)/16;y<(15*dy+7)/16 && i<2;y++)
      if( num_cross(0,dx-1,y,y,bp,cs) != 1 ) i++;
      if( i>1+(dx+8)/32 ) Break; // rnd80-Droid-Sans-Mono I
...
*/
      ...
      /* check the most left point on middle left area 3 vs. J 2013-06 */
      i =nearest_frame_vector(box1,i7,i1,x0,y0+dy/2);
      if (box1->frame_vector[i ][0]<
          box1->frame_vector[i1][0]-dx/8-1) Break; // 3
...
static wchar_t ocr0_brackets(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>1 && dy>2;){     // min 3x2
      ...
      if (    aa[0][0]+aa[1][0]-2*x>=0) Break;
      d=line_deviation(box1, aa[0][3], i1); // 2017-03: fixed >sq(1024/4);
      /* check if upper left and center point are joined directly */
...
//   if(!box1->dots)
   for(ad=d=100;dx>1 && dy>4;){     // min 3x4 '(' ')'
      DBG( wchar_t c_ask='('; )
      if (sdata->holes.num > 1) {Break;}; /* tolerant against a tiny hole */
      if (aa[0][1]>y0+dy/8 && aa[3][1]>y0+dy/8) Break; // no upper end?
      if (aa[1][1]<y1-dy/8 && aa[2][1]<y1-dy/8) Break; // no lower end?
      // check upper and lower right corner     2018-09
      if  (aa[2][1]>=y1-dy/8 && aa[2][0]>=x1-dx/8  // ( vs. l (handwritten)
       &&  aa[3][1]<=y0+dy/8 && aa[3][0]<=x1-dx/4) ad=99*ad/100; 
      if  (aa[2][1]>=y1-dy/8 && aa[2][0]>=x1-dx/8  // ( vs. l (handwritten)
       &&  aa[3][1]<=y0+dy/8 && aa[3][0]<=x1-dx/2) Break; 
 ...
 */      
   ...
   for(ad=d=98;dx>2 && dy>5;){     // min 3x(4+2)
      ...
      for (y=0;y<dy/2;y++) {
        ...
        if( y>=dy/4
         && loop(bp,   0,y,dx,cs,0,RI) // 2018-09
           -loop(bp,dx-1,y,dx,cs,0,LE)>dx/2  
           && loop(bp,   0,dy/8,dx,cs,0,RI)<dx/4 ) Break; // upper right bow
      } if (y==dy/2) Break;
      if (y==dy/2) ad=99*ad/100;   // not on ocr1809_12quest.png 7x13
      if (y==dy/2 && (dx>7 || dy>13 || 13*dx<7*dy)) Break; // bad fix
      // ToDo better add bow-detection against fail ?!
      // if( num_hole( x0, x1, y0, y1, box1->p,cs,NULL) > 0 ) Break;
      if (sdata->holes.num > 0) Break;
...
static wchar_t ocr0px(ocr0_shared_t *sdata){
   ...
   for(ad=99;dy>4 && 2*dx<dy;){     /* v0.44 */
      DBG( wchar_t c_ask='|'; )     /* solid small block */
      ...
      ac='|';
      if (!hchar) ad=98*ad/100;
      if (dx==1 && hchar && gchar) ad=99; /* 2017-07 tables */
      Setac(box1,ac,ad);
      break;
   }
   /* --- test | (vertical line, could be a i if gray and small) --- */
   for(ad=99;dy>4 && dy<12 && 3*dx<=dy;){     /* 2017-08 2x9,3x9 sslmozFP */
      DBG( wchar_t c_ask='i'; )     /* solid small block but grey gap */
      /* test if everything is filled black */
      if( get_bw(x0+dx-1,x1-dx+1,y0+dy-1,y1-dy+1,box1->p,cs,2) != 0 ) break;
      /* ToDo: check again withmodified cs? */
      if (gchar || !hchar) Break;
      if (dx>3) ad=98;
      { int x,y,a,gmi=999999,gma=0,gme=0, yma=999999;
        for (y=0;y<dy;y++) {
          for  (a=0,x=0;x<dx;x++)
             a += getpixel(box1->p, x0+x, y0+y) & ~0x0F; // 2018-09 ~marker
          if (a<gmi) { gmi=a; }
          if (a>gma) { gma=a; yma=y; }
          gme += a;
          DBG( IFV fprintf(stderr,"\nDBG i| y=%2d grey=%4d",y,a); )
        }
        DBG( IFV fprintf(stderr,"\nDBG i| yma %2d gmi,me,ma %4d %4d %4d",\
          yma,gmi,gme/dy,gma); ) /* most white column */
        if (yma>=dy/2 || y0+yma>box1->m2) Break;
        if (dy*gma<gme*2) Break; /* ToDo: better mean near blk + min near white */
      }
      ac='i';
      { int i;
        for (i=0;i<box1->num_ac;i++) /* reduce l|I */ 
          if (box1->wac[i]<100 && ad <= box1->wac[i]) box1->wac[i]--;
      }
      Setac(box1,ac,ad);
      break;
   }
...
wchar_t ocr0(struct box *box1, pix *bp, int cs){
   ...
   gchar=0;if(  // 2018-09 nums1 bad m1..m4 
               y1 > box1->m3+(box1->m4-box1->m3)/2 ) gchar=1;
   // if the char is slightly moved down correction can be done
   if ( y0<box1->m2 && y1>box1->m3 && 2*y1<box1->m3+box1->m4) // moved
   if( 2*(y0-(y1-box1->m3))<=2*box1->m2-(box1->m2-box1->m1) ) hchar=1;
   ...
   /* 2018-09 ToDo: on 5x8 "y" this is a problem bottom right 2 vectors */
   ...
   for (i=0;i<box1->num_frame_vectors[0];i++) {
     ...
     j=1; d=sq((x-x0)*128/dx)+sq((y-y1)*128/dy);  // ToDo18 see j=2-3
     if (d<aa[j][2]) { aa[j][0]=x; aa[j][1]=y; aa[j][2]=d; aa[j][3]=i; }
     /* distance to lower right end */
     j=2; d=sq((x-x1)*128/dx)+sq((y-y1)*128/dy);
     if (d<aa[j][2] ||
       (d==aa[j][2] && i>aa[j-1][3])) // 2018-09 5x8.y
                     { aa[j][0]=x; aa[j][1]=y; aa[j][2]=d; aa[j][3]=i; }
     /* distance to upper right end */
     j=3; d=sq((x-x1)*128/dx)+sq((y-y0)*128/dy);
     if (d<aa[j][2] ||
       (d==aa[j][2] && i>aa[j-1][3])) // 2018-09 5x8.z
           { aa[j][0]=x; aa[j][1]=y; aa[j][2]=d; aa[j][3]=i; }
   }
    ...
    */
   sdata.holes.num=0;  /* set by num_hole(), ToDo18 better .num_frames */
   ...
   */
   ...
   DBG( IFV fprintf(stderr,"# aa[] %d %d  %d %d  %d %d  %d %d"
                           " (4 corners)"
                           " d= %d %d %d %d vi= %d %d %d %d",
       aa[0][0]-x0,aa[0][1]-y0,aa[1][0]-x0,aa[1][1]-y0,
       aa[2][0]-x0,aa[2][1]-y0,aa[3][0]-x0,aa[3][1]-y0,
       aa[0][2],   aa[1][2],   aa[2][2],   aa[3][2],
       aa[0][3],   aa[1][3],   aa[2][3],   aa[3][3]);)
   // thin fonts may have inner and outer vector nearest to corner, 5x8.y
   ...
   if( 3*dy<box1->m4-box1->m1+2 && abs(dx-dy)<(dx+dy)/4+2
    ...
    && 5*y0>=(3*box1->m3+2*box1->m2) ){ // Jul00
     ...
     if(d==0)  // 2018-09 adding 12.5% (1/8) tolerance for handwritten
     if(  loop(box1->p,x0,y0+dy/8,x1-x0,cs,0,RI)       // top    left  vs. komma?
       <= loop(box1->p,x0,y1-dy/8,x1-x0,cs,0,RI)+dx/8  // bottom left
      ||  loop(box1->p,x1,y0+dy/8,x1-x0,cs,0,LE)       // top right
       >= loop(box1->p,x1,y1-dy/8,x1-x0,cs,0,LE)-dx/8 ) // bottom right
     { 
      bc='.';
