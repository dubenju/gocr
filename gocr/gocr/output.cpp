...
int debug_img(char *fname, struct job_s *job, int opt) {
  ...
  unsigned char *np;
  if (!job) { fprintf(stderr,"ERR job==NULL\n"); exit(1); }
  pix *pp = &job->tmp.ppo;
  ...
  if ((job->cfg.verbose&32) && job->res.lines.dx && (4&opt)/* 2018-10*/)
    for(i=0;i<pp->x;i++) {
      y=pp->y/2;
  ...
  if (3&opt) {
  for_each_data(&(job->res.boxlist)) {  /* char boxes */
    box2 = (struct box *) list_get_current(&(job->res.boxlist));
  ...
  } end_for_each(&(job->res.boxlist)); /* char boxes */
  } /* opt */
  ...
  if ((job->cfg.verbose&33) || pp->x*pp->y <= 0)
    fprintf(stderr,"# writing %s[.png] xy= %d %d\n", fname, pp->x, pp->y);
  if (pp->x*pp->y <= 0) exit(1); // generate error
  writeppm(fname, pp, opt);
  return 0;
}
