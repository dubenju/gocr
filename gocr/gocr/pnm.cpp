...
int readpgm( char *name, pix *p, int vvv){
  ...
  if( c2=='5' || c2=='2' ) /* slow PGM-RAW/ASC read pixelwise */
    for (i=0;i<nx*ny;i++) {
     ...
     // P2 was buggy until 2018-09 read 3 values per pixel, fixed by F.O.
     else fread_num(buf, bps, f1); // reading one number per pixel
     // 2018-10 Endianess.NetPBM=MSB=H,L changed from L,H
     // pic[i]=buf[bps-1]; /* store the most significant byte, L,H */
     pic[i]=buf[0]; /* store the most significant byte, MSB=H,L 2018-10 */
     // ToDo: no roundup if next bit is 1 (simpler code)
     //if (i>451+203*3800 && i<455+203*3800)
     //  fprintf(stderr,"\nDBG.rd %d,%d %3d %3d %3d - ",i%nx,i/nx,pic[i],
     //    (buf[0]&255),(buf[1]&255));
    }
